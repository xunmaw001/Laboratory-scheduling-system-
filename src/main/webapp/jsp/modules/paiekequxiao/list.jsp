...
<body>
    ...
    <div class="modal fade" id="paiekequxiaoYesnoTypesModal" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        审核：
                        <div class="col-sm-9">
                            <input type="hidden" id="paiekequxiaoYesnoTypesId">
                            <input type="hidden" id="kechengId">
                            <select id="paiekequxiaoYesnoTypes" name="paiekequxiaoYesnoTypes" class="form-control">
                       ...
                       <div class="row">
                           审核原因：
                           <textarea style="width: 450px"  id="paiekequxiaoYesnoText" name="paiekequxiaoYesnoText" class="form-control"placeholder="审核原因"></textarea>
                       </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">关闭</button>
                    <button type="button" id="receive" onclick="paiekequxiaoYesnoTypesShenhe()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
...
<!--/Pre Loader -->
        ...
        <div id="content">
            ...
            <div class="container mt-0">
                <div class="row breadcrumb-bar">
                    <div class="col-md-6">
                        <h3 class="block-title">排课取消申请管理</h3>
                    </div>
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">排课取消申请列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="widget-area-2 lochana-box-shadow">
                            <h3 class="widget-title">排课取消申请列表</h3>
    ...
    <script>
        ...
        function getDataList() {
            http("paiekequxiao/page", "GET", {
                page: pageIndex,
                limit: pageSize,
                sort: sortColumn,
                order: sortOrder,
                paiekequxiaoDelete: 1,
                //本表的
            //级联表的
        ...
        function setDataRow(item, number) {
            ...
            //排课取消审核
            var paiekequxiaoYesnoTypesCell = document.createElement('td');
            paiekequxiaoYesnoTypesCell.innerHTML = item.paiekequxiaoYesnoValue;
            row.appendChild(paiekequxiaoYesnoTypesCell);


            //审核原因
            var paiekequxiaoYesnoTextCell = document.createElement('td');
            paiekequxiaoYesnoTextCell.innerHTML = item.paiekequxiaoYesnoText;
            row.appendChild(paiekequxiaoYesnoTextCell);
            ...
            if(item.paiekequxiaoYesnoTypes == 1){
                //审核按钮
                var yesnoBtn = document.createElement('button');
                var yesnoAttr = 'openYesnoTypes(' + item.id + ','+item.kechengId+')';
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("paiekequxiao/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addpaiekequxiao", "addpaiekequxiao");
            window.location.href = "add-or-update.jsp"
        }
    ...
    //查询级联表搜索条件所有列表
            ...
            function openYesnoTypes(id,kechengId) {
                $("#paiekequxiaoYesnoTypesId").val(null)
                $("#paiekequxiaoYesnoTypesId").val(id)
                $("#kechengId").val(null)
                $("#kechengId").val(kechengId)
                $('#paiekequxiaoYesnoTypesModal').modal('show');
            }

            function paiekequxiaoYesnoTypesShenhe() {
                var mymessage = confirm("真的要确定这个审核结果吗？");
                if (mymessage == true) {
                    let data = {};
                    data["id"] = $("#paiekequxiaoYesnoTypesId").val();
                    if($("#paiekequxiaoYesnoTypes option:selected").val() == 2 || $("#paiekequxiaoYesnoTypes option:selected").val() ==3){
                        data["paiekequxiaoYesnoTypes"] = $("#paiekequxiaoYesnoTypes option:selected").val();//获取选中的审核结果
                    }else{
                        alert("请选择审核结果")
                        return false;
                    }
                    data["paiekequxiaoYesnoText"] = $("#paiekequxiaoYesnoText").val();
                    if(data.paiekequxiaoYesnoTypes == 2){
                        var paramArray = []
                        paramArray.push($("#paiekequxiaoYesnoTypesId").val())
                        httpJson("paiekequxiao/delete", "POST", paramArray, (res) => {
                            if(res.code == 0){
                                getDataList();
                                alert('操作成功');
                        ...
                        let ids = []
                            ids.push($("#kechengId").val())
                        httpJson("kecheng/delete", "POST",ids , (res) => {
                            if(res.code == 0){
                                getDataList();
                            }
                        });
                        $('#paiekequxiaoYesnoTypesModal').modal('hide');
                        $("#paiekequxiaoYesnoTypes").val(1);
                        $("#paiekequxiaoYesnoTypesId").val(null);
                        $("#paiekequxiaoYesnoText").val(null);
                    }else{
                        httpJson("paiekequxiao/update", "POST", data, (res) => {
                            if(res.code == 0){
                                alert('审核成功');
                                getDataList();
                            }else{
